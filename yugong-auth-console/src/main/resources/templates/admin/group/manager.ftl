            ...
            jQuery(document).ready(function () {
                ...
                let dtTable = dtTableDom.DataTable({
                    ...
                    ajax: function (params, callback, obj) {
                        ...
                        $.blockUI({message: '加载中...'});
                        $.post("/ajax/admin/group/pageList.htm", request, function (res, status, xhr) {
                            if (res.success) {
                                callback(res);
                            } else {
                                callback({
                    ...
                    columns: [
                        {data: "groupId", title: "组Id", width: "5em"},
                        {data: "appId", title: "应用Id", width: "5em"},
                        {data: "groupCode", title: "组标识", width: "10em"},
                        {data: "groupName", title: "组名称", width: "20em"},
                        {data: "groupDesc", title: "描述"},
                        {data: "parentGroupId", title: "父级组Id", width: "5em"},
                        {data: "createTime", title: "创建时间", width: "10em"},
                        {
                            title: "操作", width: "10em", render: function (data, type, row, meta) {
                                let html = '';
                                html += '<a class="btn btn-primary btn-xs" href="/admin/role/manager.htm?appId=' + row.groupId + '" target="_blank">角色管理</a>';
                                return html;
                            }
                        }
                    ],
