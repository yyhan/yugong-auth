                    ...
                    <div class="col-md-12">
                        <form class="form-horizontal" onsubmit="return false;">
                            <div class="form-group form-group-sm">
                                <label class="col-sm-1 control-label" for="qAppCode">应用标识</label>
                                <div class="col-sm-2">
                                    <input class="form-control" type="text" id="qAppCode">
                                </div>
                                <label class="col-sm-1 control-label" for="qAppName">应用名称</label>
                                <div class="col-sm-2">
                                    <input class="form-control" type="text" id="qAppName">
                                </div>
                            </div>
                            <div class="form-group form-group-sm">
                ...
                let dtTable = dtTableDom.DataTable({
                    ...
                    ajax: function (params, callback, obj) {
                        let request = {
                            ...
                            start: params.start,
                            appCode: $("#qAppCode").val(),
                            appName: $("#qAppName").val()
                        };

                        $.blockUI({message: '加载中...'});
                        $.post("/ajax/admin/application/pageList.htm", request, function (res, status, xhr) {
                            if (res.success) {
                                callback(res);
                            } else {
                                callback({
                    ...
                    columns: [
                        {data: "appId", title: "应用Id", width: "5em"},
                        {data: "appCode", title: "应用标识", width: "10em"},
                        {data: "appName", title: "应用名称", width: "20em"},
                        {data: "createTime", title: "创建时间", width: "10em"},
                        {
                            title: "操作", width: "10em", render: function (data, type, row, meta) {
                                let html = '';
                                html += '<a class="btn btn-primary btn-xs" href="/admin/role/manager.htm?appId=' + row.appId + '" target="_blank">角色管理</a>';
                                html += '<a class="btn btn-primary btn-xs" href="/admin/group/manager.htm?appId=' + row.appId + '" target="_blank">组管理</a>';
                                html += '<a class="btn btn-primary btn-xs" href="/admin/user/manager.htm?appId=' + row.appId + '" target="_blank">用户管理</a>';
                                return html;
                            }
                        }
                    ],
